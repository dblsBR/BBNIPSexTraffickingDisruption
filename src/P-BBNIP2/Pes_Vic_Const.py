...
def Pes_Vic_Const (G, s, t, budget, rate, obj_traffickers, Gamma_Tra, restriction):
    
    Gv_M = NetConverter_MFNIP(G);
    
    model = gp.Model("Pes_Vic_Constrained"); 
    
    gamma = model.addVars(Gv_M.edges, vtype=GRB.BINARY); 
    ...
    if restriction == 1:
        for i,j in Gv_M.edges:
            if Gv_M.edges[i,j]['trafficker'] == 1:
                model.addConstr(gamma[i,j] == 0);
            elif Gv_M.edges[i,j]['bottom'] == 1:
                model.addConstr(gamma[i,j] == 0);
    ...
    model.addConstr(gp.quicksum(Gv_M.edges[i,j]['cost']*gamma[i,j] for i,j in Gv_M.edges) <= budget);
    ...
    model.addConstrs(gp.quicksum(xv[v,u] for u in Gv_M.successors(v)) -
                      gp.quicksum(xv[u,v] for u in Gv_M.predecessors(v)) == 0 for v in G.nodes);
    
    
    # ~~~~~~~~~ Constraint on Number of Traffickers ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    ...
    model.addConstr(gp.quicksum(Gt_M.edges[i,j]['capacity']*betat[i,j] for i,j in Gt_M.edges) <= obj_traffickers);
    
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
        
    model.addConstr(xv[t,s] == obj);

    model.setObjective(gp.quicksum(Gv_M.edges[i,j]['capacity']*betav[i,j] for i,j in Gv_M.edges), GRB.MINIMIZE);
    
    
    
    for i,j in Gv_M.edges:
        gamma[i,j].start = Gamma_Tra[i,j];
