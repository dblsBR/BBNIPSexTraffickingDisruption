...
def NetConverter_MFNIP(G):
    ...
    for i,j in G.edges:
        if G.edges[i,j]['special'] == 1:
            ...
            Gm.edges[i,j]['capacity'] = 100000; # Large number compared to capacities in the network
    
    return Gm;


def Pessimistic_Constrained (G, s, t, budget, obj_revenue, Gamma_Rev, restriction):
    ...
    model.addConstr(gp.quicksum(G.edges[i,j]['capacity']*beta[i,j] for i,j in G.edges) <= obj_revenue);
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    
    model.addConstr(objVar == x1[t,s]);
    
    model.setObjective(gp.quicksum(G1.edges[i,j]['capacity']*beta1[i,j] for i,j in G1.edges), GRB.MINIMIZE);


    for i,j in G.edges:
        gamma[i,j].start = Gamma_Rev[i,j];
