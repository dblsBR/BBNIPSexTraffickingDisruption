...
def optimistic2 (G, s, t, budget, rate, obj_Pes, restriction):
    
    model = gp.Model("Optimistic2"); 
    ...
    model.addConstr(gp.quicksum(Gm.edges[i,j]['capacity']*betam[i,j] for i,j in Gm.edges) <= obj_Pes);
