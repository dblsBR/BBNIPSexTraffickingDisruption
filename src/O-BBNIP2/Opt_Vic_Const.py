...
def Opt_Vic_Const (G, s, t, budget, rate, obj_traffickers, Gamma_Tra, restriction):
    
    model = gp.Model("Opt_Victims_Constrained"); 
    ...
    Gt = NetConverter_MFNIP(Net_traffickers(G));
    ...
    model.addConstr(gp.quicksum(Gt.edges[i,j]['capacity']*betat[i,j] for i,j in Gt.edges) <= obj_traffickers);
    
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
        
    model.setObjective(gp.quicksum(z[i,j] for i,j in G.edges), GRB.MINIMIZE);
